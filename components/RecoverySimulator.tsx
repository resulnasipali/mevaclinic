...
const recoveryData = {
  ...
  hair: [
    ...
    },
    {
      ...
      protocol: "Mathematical density assessment vs. MD Harun A.'s original model.",
      badge: "Growth Surge"
    },
    {
      stage: "1 Year",
...
const RecoverySimulator = ({ type = 'bariatric', isEn = true, lang = 'en' }: { type?: string, isEn?: boolean, lang?: string }) => {
  ...
  const currentData = (recoveryData as any)[type] || recoveryData.bariatric;
    ...
    <div className="bg-white rounded-[2.5rem] shadow-2xl border border-gray-100 p-8 md:p-12 overflow-hidden relative">
      ...
      <div className="flex items-center gap-3 mb-10">
        ...
        <h3 className="text-2xl md:text-3xl font-serif font-bold text-prime">
          {tUI("Recovery Science Simulator", lang)}
        </h3>
      </div>
        ...
        <div className="space-y-6 p-6 bg-gray-50 rounded-2xl border border-gray-100 relative overflow-hidden">
          ...
          <div className="pt-4 border-t border-gray-200">
             <p className="text-[10px] font-bold text-prime uppercase tracking-widest">
                {tUI("Medical Validation: Board Review Required", lang)}
             </p>
          </div>
        </div>
      </div>
      ...
      <div className="mt-12 text-center">
         <p className="text-xs text-gray-400 font-medium italic">
            {tUI("*Timeline results are averages based on clinical trials. Individual results vary per case.", lang)}
         </p>
      </div>
    </div>
  );
