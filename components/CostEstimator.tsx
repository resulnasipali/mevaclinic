...
const CostEstimator = ({ lang = 'en' }: { lang?: string }) => {
      ...
      <div className="bg-[#0b1626] px-7 py-6">
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="text-xs font-bold text-accent uppercase tracking-widest">
            {tUI("Cost Estimator", lang)}
          </span>
        </div>
        <h3 className="text-2xl font-serif font-bold text-white">
          {tUI("Build Your Package", lang)}
        </h3>
        <p className="text-sm text-gray-400 mt-1">
          {tUI("Select your treatment and add-ons to see an estimated range.", lang)}
        </p>
      </div>
      ...
      <div className="p-7 space-y-6">
        ...
        <div>
          ...
          <div className="relative">
            ...
            >
              <option value="">{tUI("— Choose a procedure —", lang)}</option>
              {treatments.map(t => (
        ...
        {selectedTreatment && (
          <div className="bg-gray-50 rounded-2xl p-5 border border-gray-100">
            <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">
              {tUI("Estimated Package Range", lang)}
            </p>
            ...
            <p className="text-xs text-gray-400 mt-1">
              {tUI("* Indicative range. Final quote provided after clinical evaluation.", lang)}
            </p>
          </div>
        )}
        ...
        {!unlocked ? (
          <form onSubmit={handleUnlock}>
            <p className="text-xs text-gray-500 mb-3 flex items-center gap-1.5">
              ...
              {tUI("Enter your email to unlock the full price breakdown.", lang)}
            </p>
            <div className="flex gap-2">
              <input
                ...
                placeholder={tUI("your@email.com", lang)}
          ...
          <div className="space-y-2">
            <a
              href={`https://wa.me/905324675941?text=${encodeURIComponent(tUI(`Hello, I used the cost estimator and I'm interested in ${selectedTreatment}. Estimated: €${minTotal}-€${maxTotal}`, lang))}`}
            ...
            >
              {tUI("Confirm on WhatsApp", lang)} <ArrowRight size={15} />
            </a>
            <p className="text-center text-xs text-gray-400">
              {tUI("Price list sent to your email.", lang)}
            </p>
          </div>
        )}
      </div>
